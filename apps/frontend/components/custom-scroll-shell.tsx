...
export function CustomScrollShell({
  ...
  const updateMetrics = useCallback(() => {
    ...
    const nextThumbOffset =
      maxThumbOffset === 0 ? 0 : (viewport.scrollTop / maxScrollTop) * maxThumbOffset;
  ...
  }, [clearHideTimer, isDragging, scheduleTrackHide]);
  ...
  function handleThumbPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const handlePointerMove = (moveEvent: PointerEvent) => {
      ...
      const maxThumbOffset = Math.max(currentTrack.clientHeight - currentMetrics.thumbHeight, 0);
      ...
      currentViewport.scrollTop = dragStateRef.current.startScrollTop + scrollDelta;
    };
  ...
  function handleTrackPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    const maxScrollTop = viewport.scrollHeight - viewport.clientHeight;
    const maxThumbOffset = Math.max(track.clientHeight - metrics.thumbHeight, 0);
